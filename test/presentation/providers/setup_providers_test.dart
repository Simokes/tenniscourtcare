...
class MockAuthNotifier extends AuthNotifier {
  MockAuthNotifier(AsyncValue<AuthState> initialState)
    : super(StalledAuthRepository()) {
    state = initialState;
  }
}
...
void main() {
  group('setupStatusProvider', () {
    test(
      'emits loading then needsAdminSetup when admin does not exist',
      () async {
        ...
        expect(
          stream,
          emitsInOrder([SetupStatus.loading, SetupStatus.needsAdminSetup]),
        );
      },
    );

    test(
      'emits loading then needsLogin when admin exists but user is null',
      () async {
        final container = ProviderContainer(
          overrides: [
        ...
        expect(
          stream,
          emitsInOrder([SetupStatus.loading, SetupStatus.needsLogin]),
        );
      },
    );

    test(
      'emits loading then authenticated when admin exists and user is logged in',
      () async {
        ...
        final container = ProviderContainer(
          overrides: [
            adminExistsProvider.overrideWith((ref) => Future.value(true)),
            authStateProvider.overrideWith(
              (ref) => MockAuthNotifier(AsyncValue.data(AuthState(user: user))),
            ),
          ],
        );
        ...
        expect(
          stream,
          emitsInOrder([SetupStatus.loading, SetupStatus.authenticated]),
        );
      },
    );
    ...
    test('emits loading then error if admin check fails', () async {
      ...
      expect(stream, emitsInOrder([SetupStatus.loading, SetupStatus.error]));
    });
  });
}
