...
class YearlyComparisonState {
  ...
  int get totalNMinus1 => dataNMinus1.manto + dataNMinus1.sottomanto + dataNMinus1.silice;
...
final _yearNProvider = StreamProvider.autoDispose<({int manto, int sottomanto, int silice})>((ref) {
  final db = ref.watch(databaseProvider);
  final year = DateTime.now().year;
  return db.watchYearlySacksTotal(year);
});

final _yearNMinus1Provider = StreamProvider.autoDispose<({int manto, int sottomanto, int silice})>((ref) {
  final db = ref.watch(databaseProvider);
...
final yearlyComparisonProvider = Provider.autoDispose<AsyncValue<YearlyComparisonState>>((ref) {
  ...
  if (dataNMinus1Async.hasError) {
    return AsyncValue.error(dataNMinus1Async.error!, dataNMinus1Async.stackTrace!);
  }
