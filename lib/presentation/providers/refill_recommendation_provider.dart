...
final refillRecommendationProvider = FutureProvider.family<RefillRecommendation, int>((ref, terrainId) async {
  ...
  try {
    weatherContext = await weatherService.getWeatherForTerrain('club_location');
  } catch (e) {
  ...
  final lastRecharge = await db.getLastMaintenanceForTerrain(terrainId, type: 'Recharge');
  ...
  final temp = weatherContext.temperature ?? 20.0;
  // Note: Precipitation data not available in simple WeatherContext stub
  final precip = 0.0;

  // Règle météo 1 : Terre sèche
  if (temp > 25 && precip < 0.5) {
    bags += 2;
