...
Stream<List<Reservation>> userReservationsStream(
  ...
  return (db.select(db.users)..where((u) => u.firestoreUid.equals(userId)))
      ...
      .asyncExpand((user) {
        ...
        return (db.select(db.reservations)..where(
              (r) =>
                  r.userId.equals(user.id) & r.status.equals('cancelled').not(),
            ))
            .watch()
            .map((reservations) {
    ...
    saveToLocal: (items) async {
      await db.transaction(() async {
        ...
        if (isAdmin) {
          ...
          await (db.delete(
            db.reservations,
          )..where((r) => r.userId.equals(currentUser.id))).go();
        }
        ...
        for (final item in items) {
          ...
          final t = await (db.select(
            db.terrains,
          )..where((t) => t.remoteId.equals(item.terrainId))).getSingleOrNull();
